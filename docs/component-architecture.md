...
**Uzasadnienie:**
- Astro = szybkość (zero JS domyślnie) + SSR gdy potrzeba
- shadcn/ui = sprawdzone komponenty + customizacja via Tailwind
- Supabase = backend (baza danych, storage, auth)
- Tailwind = stylowanie (już w shadcn)
...
├── src/
│   ├── components/
│   │   ├── ui/              # shadcn/ui komponenty (wybrane)
│   │   ├── blocks/          # Sekcje/-bloki (Hero, Services, etc.)
│   │   ├── forms/           # Formularze (Contact, Reservation)
│   │   └── layout/          # Nawigacja, Footer, Container
│   │
│   ├── layouts/
│   │   └── Layout.astro
│   │
...
## 2. Wybrane komponenty shadcn/ui

### Minimalny zestaw (do MVP)
...
## 5. Theme configuration (Tailwind)
...
cp .env.example .env
# Edytuj .env i dodaj SUPABASE_URL i SUPABASE_ANON_KEY
```
