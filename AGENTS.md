...
## Workflow (IMPORTANT)
Pred commitom vždy spusti: `npm run build && npm run test && npm run test:e2e`
CI na GitHub beží unit + e2e automaticky, ale lokálne to treba overiť.
...
This project is indexed by GitNexus as **magicroom-mt** (474 symbols, 504 relationships, 0 execution flows). Use the GitNexus MCP tools in Kilo to understand code, assess impact, and navigate safely.

## Setup

The GitNexus MCP server is configured in `~/.config/kilo/kilo.jsonc`. After Kilo restarts, the following tools are available:

- `gitnexus_query` — search knowledge graph
...
> If tools are unavailable, run `npx gitnexus analyze` in terminal first.
...
3. `READ gitnexus://repo/magicroom-mt/process/{processName}` — trace the full execution flow step by step
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/magicroom-mt/context` | Codebase overview, check index freshness |
| `gitnexus://repo/magicroom-mt/clusters` | All functional areas |
| `gitnexus://repo/magicroom-mt/processes` | All execution flows |
| `gitnexus://repo/magicroom-mt/process/{name}` | Step-by-step execution trace |
...
This project is indexed by GitNexus as **magicroom-mt** (520 symbols, 548 relationships, 0 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
This project is indexed by GitNexus as **magicroom-mt** (531 symbols, 559 relationships, 0 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
